                            ...
                            <div class="portlet box blue">
                                <div class="portlet-title">
                                    <div class="caption">
                                       Task Detail</div>
                                       <div class="actions">
                                       <a href="<?php echo base_url()?>task/add_task"><button type="button" class="btn blue delete pull-right"><i class="fa fa-plus"></i><span> Add Task</span></button></a>
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_2">
                                        ...
                                        if(!empty($task_data))
                                        {
                                            foreach($task_data as $key)
                                            {
                                                ...
                                                ?>
                                               <tr id="xxx<?php echo $key->task_id;?>">
                                                   <td><center><input type="checkbox" class="emp_checkbox" data-emp-id="<?php echo $key->task_id; ?>"></center></td>
                                                    <td><center>
                                                        <?php echo $key->title;?><br>
                                                        
                                                         <span class="label label-sm label-info"><?php echo 'Date - '. $date;?></span></center>
                                                    </td>
                                                    <td><?php echo substr($key->description,0,50)."...";?></td>
                                                    <td width="60px;"><center>
                                                        <a href="<?php echo base_url('task/detail/'.$key->task_id); ?>" title="click here to details"><span class="label label-sm label-primary"><i class="fa fa-info-circle"></i></span></a>
                                                        <a href="<?php echo base_url('task/edit/'.$key->task_id); ?>" title="click here to edit"><span class="label label-sm label-purple"><i class="fa fa-pencil"></i></span></a></center>
                                                    </td>
                                                </tr>
                                                  <?php  $i++;
                                             } }
...
{
    var str = "task_id="+id+"&status="+status;
...
{ 
    ...
    if(r==true)
    {
        $.ajax({
           type:"POST",
           url:"<?php echo base_url('task/task_status')?>/",
           data:str,
           success:function(data)
           {  
               if(data==1000)
...
url: "<?php echo base_url('task/delete')?>",
cache:false,
data: 'task_id='+selected_values+'&status='+sts,
