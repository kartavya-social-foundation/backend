			...
			{ 
				...
				}else
				{
					...
					}else
					{
						...
						if($existing_userid != '')
						{ 
							$check_refercode = $this->db->query("SELECT referral_code FROM users WHERE referral_code = '$existing_userid' LIMIT 1")->row();
						}
						else
						{ 
							$check_refercode = 'true'; 
						}	
						//$check_refercode = $this->common_model->getDataField("referral_code","users",array('referral_code'=>$existing_userid));
						if(!empty($check_refercode))
						{
...
{		
    ...
    if(!empty($json1))
    {
	    ...
	    if(strlen($json_array->mobile_otp)==6 && !empty($json_array->mobileno) && !empty($json_array->type))
		{
			...
			}elseif($json_array->type==3)
			{
				$query_login = $this->common_model->common_getRow("users",array('email'=>$json_array->mobileno,'otp'=>hash('sha256',$json_array->mobile_otp)));
			}
	     	if($query_login)
	     	{  
				...
				if($json_array->type==2)
				{
					if($query_login->email_status != 1)
					{
			            $array_object['status'] = "failed";
						$array_object['message'] ="Please verify email address";
						header("content-type: application/json");
				...
				{
					$array_object['status'] = "1000";
					$array_object['message'] ="Your Kartvya account has been temporarily suspended as a security precaution.";
				}else
				{
					if($json_array->type == 2)
					{
					...
					{
						$update = $this->common_model->updateData("users",array('token'=>$auth_key,'email_status'=>1,'otp'=>'','update_at'=>militime),array('user_id'=>$query_login->user_id));
					}
					if($update)
					{  
			     		$final_output['status'] = 'success';
						$final_output['message'] = 'Your Mobile No. has been verified successfully';	
						$final_output['data'] = array('token'=>$auth_key);
					}
					else
					{ 
...
// 		$otp = hash('sha256',$Otp);
// 		$mobile_msg = "OTP to login your kartavya app account is ".$Otp;
// 		if( ($type == 1 && !empty($email_id) && !empty($password)) || ($type != 1 && !empty($mobile_no)) )
// 		{
// 			global $db;
...
// 			}elseif($type == 3){
// 			$query_login = $this->common_model->common_getRow("users",array('email'=>$mobile_no));
// 			}
// 			if($query_login)
// 			{	
...
// 					if($type == 1)
// 					{
// 						if(sha1($password) != $query_login->password)
// 						{
// 							$array_object['status'] = "failed";
// 							$array_object['message'] ="Password does not match";
// 							header("content-type: application/json");
// 							echo json_encode($array_object);
// 							exit;
// 						}elseif($query_login->mobile_status != 1)
// 						{
...
// 							//otp send code
// 						}
// 						$update = $this->common_model->updateData("users",array('device_token'=>$device_token,'device_id'=>$device_id,'token'=>$token,'otp'=>$otp,'update_at'=>militime),array('user_id'=>$query_login->user_id));
// 					}elseif($type==2)
// 					{
// 						$token = '';
// 						$this->common_model->sms_send($mobile_no,$mobile_msg);
// 						//otp send code
// 						$update = $this->common_model->updateData("users",array('device_token'=>$device_token,'device_id'=>$device_id,'otp'=>$otp,'update_at'=>militime),array('user_id'=>$query_login->user_id));
// 					}elseif($type==3)
// 					{
// 						//$otp = '123456';
// 						$otp = $this->randno(6);
...
// 						/*if($query_login->mobile_status != 1)
// 						{
							
// 						}else
// 						{*/
// 							$subject = "Kartavya App: Verification Code";
// 			              	$message= $otp." is your OTP to login your Kartavya App Account";
// 				            $email_from ='no-reply@kartavyaapp.com';
// 				        	$headers  = 'MIME-Version: 1.0' . "\r\n";
// 				            $headers .= 'Content-type: text/html; charset=iso-8859-1'. "\r\n";
// 				            $headers .= 'From: '.$email_from. '\r\n';
// 				            $cc = '';
				           
// 				        	@mail($mobile_no, $subject, $message,$headers);	
// 							$update = $this->common_model->updateData("users",array('device_token'=>$device_token,'device_id'=>$device_id,'otp'=>hash('sha256',$otp),'update_at'=>militime),array('user_id'=>$query_login->user_id));
// 						//}
// 					}
// 					if($update) 
// 					{
// 						$update = $this->db->query("UPDATE users SET device_token = '' WHERE device_id = '$device_id' AND user_id != ".$query_login->user_id." ");
...
// 						$array_object['data'] = array('user_id'=>$query_login->user_id,'username'=>$query_login->username,'usersurname'=>$query_login->user_surname,'useremail'=>$query_login->email,'token'=>$token,'mobile_code'=>$query_login->mobile_code,'mobileno'=>$query_login->mobileno,'profile_pic'=>$image,'country_id'=>$query_login->country_id,'country_name'=>$query_login->country_name,
// 						'state_id'=>$query_login->state_id,'state_name'=>$query_login->state_name,'pincode'=>$query_login->pincode,'referral_code'=>$query_login->referral_code,'user_unique_id'=>$query_login->user_unique_id,'type'=>$type,'dob'=>$query_login->dob,'gender'=>$query_login->gender,'device_token'=>$query_login->device_token,'device_id'=>$query_login->device_id,'aboutme'=>$query_login->about_me);
// 					}
// 				}
// 			}else
// 			{
