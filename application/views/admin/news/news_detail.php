                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet box green">
                                <div class="portlet-title">
                                    ...
                                    <?php
                                        if($this->uri->segment(2) =='user_news')
                                        { ?>
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_2">
                                        <thead>
                                           <tr>
                                                ...
                                                <?php if($this->uri->segment(2) == 'user_news')
                                                { ?>
                                                <th><center>Post By</center></th>
                                                <?php }else { echo '';} ?>
                                                <th width="85px"><center>Status</center></th>
                                                <th width="70px"><center>Action</center></th>
                                            </tr>
                                        </thead>
                                        <tfoot>
                                           <tr>
                                        ...
                                        if(!empty($news_data))
                                        {
                                            foreach($news_data as $key)
                                            {
                                                ...
                                                ?>
                                                <tr id="xxx<?php echo $key->news_id;?>">
                                                    <td><center><input type="checkbox" class="emp_checkbox" data-event-id="<?php echo $key->news_id;?>"></center> </td>
                                                    <td><center>
                                                        <?php echo $key->title;?><br>
                                                        <span class="label label-sm label-success badge"><?php echo 'Date - '. $date;?></span></center>
                                                    </td>
                                                    <?php if($this->uri->segment(2) == 'user_news')
                                                    { 
                                                      $user_detail = $this->common_model->common_getRow('users',array('user_id'=>$key->user_id));?>

                                                     <td><center><?php if(isset($user_detail->username) && isset($user_detail->user_surname)){ echo $user_detail->username.' '.$user_detail->user_surname;  } else { echo 'Deleted User'; } ?></center></td>
                                                    
                                                    <?php } else { echo '';}?>
                                                    <td><center><?php if($key->admin_status == 0){ echo 'Unverified';}else if($key->admin_status == 1){ echo 'Verified';}?></center></td>
                                                    <td width="60px;">
                                                        <a href="<?php echo base_url('news/detail/'.$key->news_id) ?>" title="click here to details"><span class="label label-sm label-primary"><i class="fa fa-info-circle"></i></span></a>
                                                        <a href="<?php echo base_url('news/edit/'.$key->news_id) ?>" title="click here to edit"><span class="label label-sm label-purple"><i class="fa fa-pencil"></i></span></a>
                                                    </td>
...
{ 
    var str = "news_id="+id+"&status="+status;
    var r = confirm('Are you really want to change status?');
    if(r==true)
    {
        $.ajax({
          type:"POST",
           url:"<?php echo base_url('news/news_status')?>/",
           data:str,
           success:function(data)
           {  
               if(data==1000)
...
var news = [];
$(".emp_checkbox:checked").each(function() { 
news.push($(this).data('event-id'));
});
if(news.length <=0) { alert("Please select records."); } else { WRN_PROFILE_DELETE = "Are you sure you want to reject "+(news.length>1?"these":"this")+" row?";
var checked = confirm(WRN_PROFILE_DELETE);
if(checked == true) {
var selected_values = news.join(",");
$.ajax({
type: "POST",
url: "<?php echo base_url('news/reject_news')?>",
cache:false,
data: 'news_id='+selected_values,
success: function(response) {
// remove deleted employee rows
var news_ids = response.split(",");
for (var i=0; i < news_ids.length; i++)
{ 
    var str = $.trim(news_ids[i]);
    var idz = "xxx"+str;
    $("#"+idz).hide();
}
...
if(news.length <=0) { alert("Please select records."); } else { WRN_PROFILE_DELETE = "Are you sure you want to active "+(news.length>1?"these":"this")+" row?";
var checked = confirm(WRN_PROFILE_DELETE);
if(checked == true) {
var selected_values = news.join(",");
$.ajax({
type: "POST",
url: "<?php echo base_url('news/active_news')?>",
cache:false,
data: 'news_id='+selected_values,
success: function(response) { 

  var url = "<?php echo base_url('news/user_news');?>";
