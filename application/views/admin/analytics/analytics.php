                ...
                <div class="page-content">
                    <div class="page-head">
                         <div class="page-title">
                            <h1>User Analytics<small>User Analytics</small></h1>
                        </div>
                    </div>
                   <div class="row">
                            ...
                            <div class="portlet light "style="border: 1px solid #84b4ad">
                                <div class="portlet-title">
                                    <div class="caption col-md-12">
                                        <div class="col-md-6" style="margin:0.5em 0;">
                                          <span class="bold uppercase" style="color:#1B7310" ><b><?php $user_value = 'Total';
                                            if(!empty($total_count1[0]->total_count)) 
                                            {
                                               echo $total_count1[0]->total_count;
                                            }
                                            else{ echo '0'; } 
                                            if(isset($male))
                                            {    
                                            ...
                                            else if(isset($female))
                                            {
                                               $user_value = 'Female'; 
                                            }
                                            else if(isset($verified))
                                            {
                                               $user_value = 'Verified'; 
                                            }
                                            else if(isset($unverified)) 
                                            {
                                                $user_value = 'Unverified';
                                            }
                                            else if(isset($employed)) 
                                            {
                                                $user_value = 'Employed';
                                            }  
                                            else if(isset($unemployed)) 
                                            {
                                                $user_value = 'Unemployed';
                                            }
                                            else if(isset($married)) 
                                            {
                                                $user_value = 'married';
                                            }  
                                            else if(isset($unmarried)) 
                                            {
                                                $user_value = 'unmarried';
                                            }    
                                            ?> </b> <?php echo $user_value; ?> Resister User </span>
                                           
                                        </div>
                                        <div class="col-md-6">
                                        <form class="pull-right" method="post" action="<?php echo base_url().'analytics';?>" onsubmit="return check_value()">

                                         <select name="advance_data" id="mon" class="btn btn-default btn-circle btn-outline">

                                            <option value="">Select</option>
                                            <option value="male"<?php if(isset($male)){ echo 'selected';}?>>Male</option>
                                            <option value="female"<?php if(isset($female)){ echo 'selected';}?>>Female</option>
                                            <option value="verified"<?php if(isset($verified)){ echo 'selected';}?>>Verified</option>
                                            <option value="unverified"<?php if(isset($unverified)){ echo 'selected';}?>>Unverified</option>
                                            <option value="employed"<?php if(isset($employed)){ echo 'selected';}?>>Employed</option>
                                            <option value="unemployed"<?php if(isset($unemployed)) { echo 'selected';}?>>Unemployed</option>
                                            <option value="married"<?php if(isset($married)) { echo 'selected';}?>>Married</option>
                                            <option value="unmarried"<?php if(isset($unmarried)) { echo 'selected';}?>>Unmarried</option>
                                        </select>    
...
{ 
    var str = "user_id="+id;
    var r = confirm('Are you really want to verify?');
    if(r==true)
    {
        $.ajax({
          type:"POST",
           url:"<?php echo base_url('dashboard/verify_user')?>/",
           data:str,
...
{ 
   ...
   if(m == '')
   {
        document.getElementById('error').innerHTML ='Filter and Year required';
        return false;
   }
   else if(y == '')
   {
...
var chart = AmCharts.makeChart("chartdiv", {
  ...
  <?php 
    ...
    for ($i=0; $i < $number ; $i++) { 
        $d = $mons[$i];
        $color = '#990000';
        if($i==0 || $i < 4){ $img = "https://www.amcharts.com/lib/images/faces/A04.png";}
        elseif($i==4 || $i < 8){ $img = "https://www.amcharts.com/lib/images/faces/C02.png"; }
        elseif($i==8 || $i < 12){ $img = "https://www.amcharts.com/lib/images/faces/D02.png"; }
        else{ $img = "https://www.amcharts.com/lib/images/faces/E01.png"; }
        if(isset($colorarr[0][$i])){ $color = $colorarr[0][$i]; }

        if(isset($user_count[$i]))
        {  
            ?>{ 
            "date": "<?php echo $d;?>",
            "visits":"<?php echo $user_count[$i];?>",
