                                        ...
                                        if(!empty($joining_request))
                                        {
                                            ...
                                            {?>
                                               <tr id="xxx<?php echo $key['join_id'];?>">
                                                    ...
                                                    <td>
                                                        <?php echo $key['username'].' '.$key['user_surname'];?><br>
                                                    </td>
                                                    <td>
                                                        <?php echo $key['title'];?><br>
                                                    </td>
                                                    <td>
                                                        <?php echo $key['join_date'];?><br>
                                                    </td>
                                                    <td>
                                                       <?php if($key['admin_status'] == 1){ echo 'Joined';} else{ echo 'Pending';} ?>
                                                    </td>
...
function changestatus(status,id,user_id,task_title)
{ 
    var str = "join_id="+id+"&status="+status+"&user_id="+user_id+"&title="+task_title;
    var r = confirm('Are you really want to change status?');
    if(r==true)
    {
        $.ajax({
          type:"POST",
           url:"<?php echo base_url('task/join_status')?>/",
           data:str,
...
$.ajax({
type: "POST",
url: "<?php echo base_url('task/delete_user_request')?>",
cache:false,
...
$.ajax({
type: "POST",
url: "<?php echo base_url('task/join_status1')?>",
cache:false,
data: "join_id="+selected_values,
success: function(response) {  
